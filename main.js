...
const changeStroke = document.querySelector("#rangeInput");
const bgVideo = document.querySelector("#bgVideo");

// hide loader and after effect option form
...
const getSvg = (image) => {
	return new Promise(async (resolve, reject) => {
		...
		whiteBgImage.addEventListener("load", async (e) => {
			...
			let pngToSvg = await fetch(`https://ffmpeg.wedios.co/api/png-to-svg?access_key=Pr28qGvg6XvbLLeYZbVs9KaYMtNc6mJQP5aq`, {
				method: "post",
				body: fd,
			})
				.then((e) => e.json())
			...
			targetPath.setAttribute("stroke", "#00ffee");
			targetPath.setAttribute("stroke-width", "4px");
			targetPath.setAttribute("filter", "url(#neon)");

			changeColor.addEventListener("input", (e) => {
				targetPath.setAttribute("stroke", e.target.value);
				document.querySelector("#glowColor").setAttribute("flood-color", e.target.value);
			});
			changeStroke.addEventListener("input", (e) => {
				targetPath.setAttribute("stroke-width", e.target.value.concat("px"));
			});
